...
function Layout() {
  ...
  return (
    <div
      className={`relative flex flex-col w-full divide-y h-dvh min-h-lvh bg-secondary-800 font-Quicksand divide-secondary-900 ${theme.value}`}
    >
      <Topmenu />
      <div className="flex flex-col flex-1 divide-y divide-secondary-900">
        <div className="flex-1 h-full max-h-[calc(100vh_-_3.25rem)]">
          ...
          <div className="flex h-full max-w-[100vw] max-h-full overflow-y-auto">
            ...
            <div className="relative flex flex-col flex-1 font-sans #divide-y divide-secondary-900 md:max-w-[calc(100vw_-_280px)] overflow-x-hidden">
              <div
                className={`top-0 flex w-full h-8 overflow-x-auto border-b border-secondary-900 bg-secondary-800 divide-secondary-900 scrollbar-hide ${
                  hideTopPanel && "hidden"
                }`}
              >
                ...
                {visitedRoutes.map((item) => (
                  <div
                    ...
                    key={item.href}
                    className={`flex flex-shrink-0 cursor-pointer select-none !flex-0 items-center w-56 px-2 text-secondary-300 border-r border-secondary-900 ${
                      pathname == item.href && "bg-secondary-700/50"
                    }`}
                  >
                    <div className="w-5 h-5"></div>
                    <p className="flex-1 text-xs truncate sm:text-sm first-letter:uppercase">
