...
import { useSpotifyContext } from './context/SpotifyContextProvider';

function Layout() {
    const location = useLocation();
    const navigate = useNavigate();
    const { hash, pathname, search } = location;
    const { visitedRoutes, RemoveRoute } = useSpotifyContext();
                ...
                <div className='flex-1 h-full max-h-[calc(100vh_-_3.25rem)]'>
                    ...
                    <div className="flex h-full max-w-[100vw] max-h-full overflow-y-auto">
                        ...
                        <div className="relative flex flex-col flex-1 font-sans divide-y divide-slate-900 md:max-w-[calc(100vw_-_280px)] overflow-x-hidden">
                            <div className="top-0 flex w-full h-8 overflow-x-auto bg-slate-800 divide-slate-900 scrollbar-hide">
                                ...
                                {/* {
                                    visitedRoutes.map((item) => <NavLink key={item.id} to={item.href} className={({ isActive }) => `flex flex-shrink-0 !flex-0 items-center w-56 px-2 text-gray-300 border-r border-slate-900 ${isActive && 'bg-slate-700/50'}`}>
                                        <div className='w-5 h-5'>
