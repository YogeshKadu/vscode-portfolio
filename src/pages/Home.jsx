  ...
  return (
    <div className="min-h-full">
      <div className="w-full #max-w-5xl p-8 sm:p-20 xl:p-28">
        <h1 className="mb-1 text-3xl font-semibold text-slate-300">Yogesh Vasant Kadu</h1>
        <p className="mb-3 text-sm capitalize text-slate-400">
          senior software developer
        </p>
        <div className="mb-8 text-sm antialiased text-slate-400">
          <ul className="pl-5">
        ...
        <div className="flex flex-col-reverse flex-wrap max-w-4xl gap-2 md:flex-row">
          <div className="flex-1">
            <div>
              <h4 className="flex mb-1 text-slate-300">Start with</h4>
            ...
            <div>
              <h4 className="mt-5 mb-2 text-slate-300">Resents</h4>
              <div className="text-sm">
                {
                  visitedList.length == 0 && <div className="text-slate-500">No recent activity! <span className="font-Noto">🙁</span></div>
                }
                {
                  visitedList.map((item) => <div key={item.href} className="text-slate-400"><span className="mr-1 text-primary-400">{item.name.split('.')[0]}</span> <span className="italic lowercase">src/{item.name}</span></div>)
                }
              </div>
            </div>
          </div>
          <div className="flex-1">
            <div>
              {/* <h4 className="mb-2 text-slate-300">Technologies</h4> */}
              {/* <h4 className="mb-2 text-slate-300">CTO</h4> */}
              <div className="flex gap-2 text-sm">
                <button className="h-8 px-5 rounded-full text-slate-950 bg-primary-500 hover:bg-primary-600" title="get in touch form">
                  Get in touch
                </button>
                <button title="Resume 😂" arial-label="download resume" className="grid w-8 h-8 rounded-full aspect-square place-content-center bg-primary-500 hover:bg-primary-600 text-slate-800">
